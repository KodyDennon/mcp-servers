...
export function resolveDeviceFromQuery(devices, query, fallbackName) {
    if (!devices.length) {
    ...
    if (normalizedQuery) {
        const byUdid = devices.find((device) => normalize(device.udid) === normalizedQuery);
        if (byUdid) {
            return { device: byUdid, reason: "udid" };
        }
        const byExactName = devices.find((device) => normalize(device.name) === normalizedQuery);
        if (byExactName) {
            return { device: byExactName, reason: "name" };
        }
        const byPartial = devices.find((device) => normalize(device.name).includes(normalizedQuery));
        if (byPartial) {
    ...
    if (fallbackName) {
        ...
        const fallbackDevice = devices.find((device) => normalize(device.name) === normalizedFallback) ??
            devices.find((device) => normalize(device.name).includes(normalizedFallback));
        if (fallbackDevice) {
