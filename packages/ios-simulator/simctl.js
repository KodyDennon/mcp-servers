...
export class Simctl {
    ...
    async run(args, options = {
        ...
        try {
            const result = await execFileAsync(this.executable, [...this.defaultArgs, ...args], options);
            return {
            ...
            throw new SimctlCommandError(`simctl command failed: ${args.join(" ")}`, args, execError.stdout?.toString() ?? "", execError.stderr?.toString() ?? "", typeof execError.code === "number" ? execError.code : null);
        }
    }
    async listDevices(kind = "available") {
        ...
        const devices = [];
        for (const [runtimeIdentifier, deviceList] of Object.entries(parsed.devices ?? {})) {
            ...
            for (const device of deviceList) {
                devices.push({
                    ...
                    state: device.state,
                    isAvailable: device.isAvailable ?? device.availability === "(available)",
