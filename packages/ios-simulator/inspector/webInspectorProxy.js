...
export class WebInspectorProxy {
    ...
    async waitUntilHealthy() {
        const timeoutAt = Date.now() + this.options.config.inspectorConnectTimeoutMs;
        ...
        throw new Error(`WebKit inspector proxy did not become ready on port ${this.port}`);
    }
    async spawnProxy() {
        if (this.process) {
            return;
        ...
        this.process.on("exit", (code, signal) => {
            console.error(`WebKit proxy exited with code ${code ?? "unknown"} signal ${signal ?? "unknown"}`);
            this.process = null;
            this.ready = false;
        });
    ...
    async listTargets() {
        ...
        const response = await fetch(`${this.baseUrl}/json`);
        if (!response.ok) {
            throw new Error(`Failed to read WebKit inspector targets: ${response.status} ${response.statusText}`);
        }
