...
const deviceDescription = z
    .string()
    .describe("Optional simulator name/UDID. Defaults to IOS_SIM_DEFAULT_DEVICE.");
export const installAppTool = {
    name: "installSimulatorApp",
    description: "Install a .app or .ipa bundle into the selected simulator. Useful for sideloading dev builds before automated testing.",
    input_schema: z
        .object({
        appPath: z
            .string()
            .min(1)
            .describe("Absolute path to the .app directory or .ipa/.xcarchive payload you want to install."),
...
export const uninstallAppTool = {
    ...
    description: "Remove an installed application from the simulator by bundle identifier.",
    input_schema: z
        .object({
        bundleId: z
            .string()
...
export const launchAppTool = {
    ...
    description: "Launch an installed app using simctl (great for bootstrapping native test sessions or deep links).",
    input_schema: z
        .object({
...
export const tapTool = {
    ...
    description: "Simulate a tap/press gesture at x/y coordinates via WebDriverAgent.",
    input_schema: z
        .object({
        ...
        bundleId: z
            .string()
            .describe("Optional bundle ID to attach the automation session to (defaults to SpringBoard).")
            .optional(),
        device: deviceDescription.optional(),
    })
        .strip(),
...
export const swipeTool = {
    ...
    description: "Perform a swipe/drag gesture for scrolling or reveal actions using WebDriverAgent.",
    input_schema: z
        .object({
        fromX: z.number(),
        fromY: z.number(),
...
export const typeTextTool = {
    ...
    description: "Send Unicode text via the simulator keyboard using WebDriverAgent. Useful for filling forms programmatically.",
    input_schema: z
        .object({
...
export const pressButtonTool = {
    ...
    description: "Emulate hardware button presses (home, lock, volume) via WebDriverAgent.",
    input_schema: z
        .object({
        button: z
...
export const hierarchyTool = {
    ...
    description: "Fetch the current accessibility/UI hierarchy (XML) from WebDriverAgent for debugging selectors.",
    input_schema: z
        .object({
        bundleId: z.string().optional(),
        device: deviceDescription.optional(),
...
export async function handleAutomationToolCall(name, args, managers) {
    switch (name) {
        case installAppTool.name: {
            const input = installAppTool.input_schema.parse(args);
            const result = await managers.simulator.installApp(input.appPath, input.device);
            return {
                device: {
        ...
        case uninstallAppTool.name: {
            ...
            const result = await managers.simulator.uninstallApp(input.bundleId, input.device);
            return {
                device: {
                    name: result.device.name,
                    udid: result.device.udid,
        ...
        case launchAppTool.name: {
            ...
            const result = await managers.simulator.launchApp(input.bundleId, input.device, input.arguments);
            return {
                device: {
                    name: result.device.name,
                    udid: result.device.udid,
        ...
        case terminateAppTool.name: {
            ...
            const result = await managers.simulator.terminateApp(input.bundleId, input.device);
            return {
                device: {
                    name: result.device.name,
                    udid: result.device.udid,
