...
export const listSimulatorsTool = {
    name: "listSimulators",
    description: "List available iOS simulator devices, including runtime, state, and UDID.",
    input_schema: z
        .object({
        includeUnavailable: z
            .boolean()
            .describe("When true, include simulators that are currently unavailable.")
            .optional(),
    })
        .strip(),
...
export const bootSimulatorTool = {
    name: "bootSimulator",
    description: "Boot (or focus) a simulator by name or UDID and bring the Simulator window to the front.",
    input_schema: z
        .object({
        device: z
            .string()
            .describe("Optional simulator name, UDID, or the literal word 'booted'. Defaults to IOS_SIM_DEFAULT_DEVICE.")
            .optional(),
    })
        .strip(),
...
export const shutdownSimulatorTool = {
    ...
    input_schema: z
        ...
        device: z
            .string()
            .describe("Simulator name/UDID or 'booted'. Defaults to the active simulator.")
            .optional(),
    })
        .strip(),
    output_schema: z.object({
...
export const openUrlTool = {
    ...
    description: "Open a URL inside Safari on the booted simulator. Boot happens automatically if needed.",
    input_schema: z
        .object({
...
export const screenshotTool = {
    ...
    description: "Capture a screenshot from the simulator display and return a base64 payload.",
    input_schema: z
        .object({
...
export async function handleSimulatorToolCall(name, args, manager) {
    switch (name) {
        case listSimulatorsTool.name: {
            const { includeUnavailable } = listSimulatorsTool.input_schema.parse(args);
        ...
        case openUrlTool.name: {
            ...
            const result = await manager.openUrl(url, device);
            return {
                url: result.url,
                device: {
                    name: result.device.name,
        ...
        case screenshotTool.name: {
            ...
            const result = await manager.captureScreenshot(device, format);
            return {
