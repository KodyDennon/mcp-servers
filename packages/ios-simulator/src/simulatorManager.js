...
export class SimulatorManager {
    config;
    simctl;
    constructor(config, simctl = new Simctl()) {
        this.config = config;
        this.simctl = simctl;
    }
    async listDevices(includeUnavailable = false) {
        const devices = await this.simctl.listDevices(includeUnavailable ? "all" : "available");
        return includeUnavailable
            ? devices
            : devices.filter((device) => device.isAvailable);
    }
    async resolveDevice(query) {
        const devices = await this.listDevices();
        const { device } = resolveDeviceFromQuery(devices, query, this.config.defaultDeviceName);
        return device;
    }
    async waitForDeviceState(udid, desiredState) {
        ...
        throw new Error(`Timed out waiting for simulator ${udid} to reach state ${desiredState}`);
    }
    async ensureSimulatorAppVisible(device) {
        try {
