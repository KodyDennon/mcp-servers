...
const ConfigSchema = z.object({
  defaultDeviceName: z
    ...
    .describe(
      "Default simulator device to boot when no explicit device is provided.",
    ),
  screenshotDir: z
    .string()
  ...
  iosWebkitProxyBinary: z
    ...
    .describe(
      "Binary that exposes the WebKit Remote Debugging Protocol for the simulator.",
    ),
  ...
  webDriverAgentProjectPath: z
    ...
    .describe(
      "Path to the checked-out WebDriverAgent project (used for native automation).",
    ),
...
let cachedConfig: SimulatorConfig | null = null;

export async function loadConfig(): Promise<SimulatorConfig> {
  if (cachedConfig) {
    return cachedConfig;
  }
  ...
  const parsed = ConfigSchema.parse({
    ...
    iosWebkitProxyBinary:
      process.env.IOS_WEBKIT_DEBUG_PROXY_BINARY || undefined,
    iosWebkitProxyPort: numberFromEnv(process.env.IOS_WEBKIT_DEBUG_PROXY_PORT),
    inspectorConnectTimeoutMs: numberFromEnv(
      process.env.IOS_WEBKIT_CONNECT_TIMEOUT_MS,
    ),
    inspectorReadyPollIntervalMs: numberFromEnv(
      process.env.IOS_WEBKIT_READY_INTERVAL_MS,
    ),
    webDriverAgentProjectPath: process.env.IOS_WDA_PROJECT_PATH || undefined,
    webDriverAgentDerivedDataPath:
      process.env.IOS_WDA_DERIVED_DATA || undefined,
    webDriverAgentPort: numberFromEnv(process.env.IOS_WDA_PORT),
    webDriverAgentStartupTimeoutMs: numberFromEnv(
      process.env.IOS_WDA_STARTUP_TIMEOUT_MS,
    ),
    webDriverAgentSessionTimeoutMs: numberFromEnv(
      process.env.IOS_WDA_SESSION_TIMEOUT_MS,
    ),
    webDriverAgentDefaultBundleId:
      process.env.IOS_WDA_DEFAULT_BUNDLE_ID || undefined,
