...
import type { WebInspectorTarget } from "../inspector/index.js";
...
export async function handleInspectorToolCall(
  ...
  switch (name) {
    case listInspectorTargetsTool.name: {
      ...
      const targets: WebInspectorTarget[] = await inspector.listTargets(device);
