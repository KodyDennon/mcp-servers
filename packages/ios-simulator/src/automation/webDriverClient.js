    ...
    async request({ method, path, body }) {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`WebDriverAgent returned non-JSON response for ${method} ${path}: ${text}`);
            }
            if (!response.ok) {
                const errorValue = data.value;
                throw new Error(`WebDriverAgent error (${response.status}): ${errorValue?.message || errorValue?.error || text}`);
            }
            ...
            if (value &&
                ...
                throw new Error(`WebDriverAgent command failed: ${error ?? "unknown"} ${message ?? ""}`.trim());
            }
            return value;
        }
        finally {
