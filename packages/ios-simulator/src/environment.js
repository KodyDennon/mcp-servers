...
async function ensureWebDriverAgent(projectPath, failures) {
    try {
        ...
        console.error(`WebDriverAgent project missing at ${projectPath}. Attempting to clone ${WDA_REPO_URL} automatically...`);
    }
    await recordFailure(failures, async () => {
...
export async function ensureEnvironment(required) {
    if (process.platform !== "darwin") {
        throw new Error("The iOS Simulator MCP server only runs on macOS because it depends on Xcode.");
    }
    const failures = [];
    await recordFailure(failures, async () => {
    ...
    if (failures.length > 0) {
        throw new Error(`Unable to start iOS Simulator MCP server because required tooling is missing:\n\n${failures
            .map((failure) => `- ${failure}`)
