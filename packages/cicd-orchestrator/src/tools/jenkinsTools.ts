...
function getJenkinsApi() {
  ...
  const auth = process.env.JENKINS_USER && process.env.JENKINS_TOKEN
    ? {
...
export async function handleJenkinsToolCall(name: string, args: Record<string, unknown>) {
  ...
  switch (name) {
    ...
    case "jenkins_get_job": {
      const { job_name } = args as { job_name: string };
      const response = await jenkins.get(`/job/${encodeURIComponent(job_name)}/api/json`);
      return {
        content: [
          {
            type: "text" as const,
    ...
    case "jenkins_trigger_build": {
      const { job_name, parameters } = args as {
        job_name: string;
    ...
    case "jenkins_get_build": {
      const { job_name, build_number } = args as { job_name: string; build_number: number };
    ...
    case "jenkins_stop_build": {
      const { job_name, build_number } = args as { job_name: string; build_number: number };
      await jenkins.post(`/job/${encodeURIComponent(job_name)}/${build_number}/stop`);
      return {
        content: [
          {
            type: "text" as const,
