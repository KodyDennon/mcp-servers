...
export async function handleGitLabToolCall(name, args) {
    if (!process.env.GITLAB_TOKEN) {
        throw new Error("GITLAB_TOKEN environment variable is required");
    }
    switch (name) {
        case "gitlab_list_pipelines": {
            const { project_id, status, limit = 10 } = args;
        ...
        case "gitlab_get_pipeline": {
            ...
            const response = await gitlabApi.get(`/projects/${encodeURIComponent(project_id)}/pipelines/${pipeline_id}`);
            return {
                content: [
                    {
                        type: "text",
        ...
        case "gitlab_trigger_pipeline": {
            ...
            const response = await gitlabApi.post(`/projects/${encodeURIComponent(project_id)}/pipeline`, { ref, variables });
            return {
                content: [
                    {
                        type: "text",
        ...
        case "gitlab_cancel_pipeline": {
            const { project_id, pipeline_id } = args;
            await gitlabApi.post(`/projects/${encodeURIComponent(project_id)}/pipelines/${pipeline_id}/cancel`);
            return {
                content: [
                    {
                        type: "text",
        ...
        case "gitlab_retry_pipeline": {
            const { project_id, pipeline_id } = args;
            const response = await gitlabApi.post(`/projects/${encodeURIComponent(project_id)}/pipelines/${pipeline_id}/retry`);
            return {
                content: [
                    {
                        type: "text",
...
//# sourceMappingURL=gitlabTools.js.map