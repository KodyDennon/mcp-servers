  ...
  switch (name) {
      ...
      try {
        ...
        try {
          const packageJson = JSON.parse(
            await readFile(resolve(directory, "package.json"), "utf-8"),
          );
        ...
        try {
          const pythonVersion = await readFile(
            resolve(directory, ".python-version"),
            "utf-8",
          );
        ...
        try {
          const rubyVersion = await readFile(
            resolve(directory, ".ruby-version"),
            "utf-8",
          );
          ...
          content: [
            {
              type: "text",
              text: JSON.stringify(
                { requirements, detected: Object.keys(requirements).length },
                null,
                2,
              ),
            },
    ...
    case "runtime_list_node_versions": {
      try {
        ...
        throw new Error(
          `Failed to list Node.js versions. Is nvm installed? ${error}`,
        );
      }
    }
    ...
    case "runtime_list_python_versions": {
      try {
        ...
        throw new Error(
          `Failed to list Python versions. Is pyenv installed? ${error}`,
        );
      }
    }
