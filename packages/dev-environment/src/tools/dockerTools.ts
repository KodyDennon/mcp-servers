...
export async function handleDockerToolCall(name: string, args: Record<string, unknown>) {
  switch (name) {
    case "docker_list_containers": {
      const { all = true } = args as { all?: boolean };
    ...
    case "docker_compose_up": {
      const { directory, detach = true } = args as { directory: string; detach?: boolean };
