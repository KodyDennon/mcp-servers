...
export async function handleRuntimeToolCall(name: string, args: Record<string, unknown>) {
  switch (name) {
      ...
      try {
        ...
        try {
          ...
          if (packageJson.engines) {
            if (packageJson.engines.node) requirements.nodeEngine = packageJson.engines.node;
            if (packageJson.engines.npm) requirements.npm = packageJson.engines.npm;
          }
        } catch {}
