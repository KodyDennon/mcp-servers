  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name.startsWith("runtime_")) {
        const { handleRuntimeToolCall } = await import(
          "./tools/runtimeTools.js"
        );
      ...
      if (name.startsWith("devcontainer_")) {
        const { handleDevContainerToolCall } = await import(
          "./tools/devcontainerTools.js"
        );
        return await handleDevContainerToolCall(name, args || {});
      }
      // Service management tools
      if (name.startsWith("service_")) {
        const { handleServiceToolCall } = await import(
          "./tools/serviceTools.js"
        );
