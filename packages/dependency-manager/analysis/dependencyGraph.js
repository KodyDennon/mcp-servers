...
export class DependencyGraphAnalyzer {
   ...
   */
  async analyzeGraph(projectPath = process.cwd()) {
  ...
  async buildGraph(projectPath) {
    ...
    for (const [name, version] of Object.entries(deps)) {
      await buildNode(root, name, version, 1, false, false);
    }
    for (const [name, version] of Object.entries(devDeps)) {
      if (!deps[name]) {
        // Don't duplicate if already in deps
        await buildNode(root, name, version, 1, true, false);
      }
    }
    return {
      root,
    ...
    for (const [packageName, nodes] of graph.allNodes) {
      ...
      if (versions.size > 1) {
        ...
        duplicates.push({
          ...
          impact: {
            bundleSize:
              versions.size > 3 ? "high" : versions.size > 1 ? "medium" : "low",
   ...
   * Check peer dependency conflicts
   */
  async checkPeerConflicts(graph, projectPath) {
    const conflicts = [];
    const checkNode = async (node) => {
      try {
        const metadata = await npmClient.getVersionMetadata(
          node.name,
          node.version,
        );
        ...
        for (const [peerName, peerVersion] of Object.entries(peerDeps)) {
          ...
          if (!installedNodes || installedNodes.length === 0) {
            ...
            const satisfied = installedNodes.some((n) =>
              semver.satisfies(n.version, peerVersion),
            );
            if (!satisfied) {
              conflicts.push({
                package: node.name,
   ...
   */
  analyzeTransitiveImpacts(graph) {
    const impacts = new Map();
    const analyzeNode = (node, directParent) => {
    ...
    return Array.from(impacts.values()).sort(
  ...
  generateRecommendations(issues) {
    ...
    for (const circular of issues.circularDependencies) {
      recommendations.push({
        ...
        estimatedImpact:
          "Prevent potential runtime issues and improve maintainability",
      });
    }
   ...
   */
  visualizeTree(graph, maxDepth = 3) {
    const lines = [];
    const addNode = (node, prefix, isLast) => {
