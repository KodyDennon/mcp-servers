...
import { BreakingChangeDetector } from "./breakingChanges.js";
...
export class AIAnalysisEngine {
   ...
   * Analyze security
   */
  async analyzeSecurity(packageName, currentVersion, latestVersion) {
    ...
    const fixAvailable = vulnerabilityReport.osv.some((v) =>
      v.affected?.some((a) =>
        a.ranges?.some((r) =>
          r.events?.some((e) => e.fixed && semver.gte(latestVersion, e.fixed)),
        ),
      ),
    );
    return {
   ...
   * Analyze quality
   */
  async analyzeQuality(packageName) {
   ...
   * Analyze maintenance
   */
  async analyzeMaintenance(packageName) {
    ...
    const daysSinceLastPublish = lastPublish
      ? Math.floor(
          (Date.now() - new Date(lastPublish).getTime()) /
            (1000 * 60 * 60 * 24),
        )
      : 0;
    // Calculate release frequency
    let releaseFrequency = "very-low";
    ...
    return {
      ...
      commitActivity:
        maintenanceInfo.commitsFrequency > 50
          ? "active"
          : maintenanceInfo.commitsFrequency > 20
            ? "moderate"
            : maintenanceInfo.commitsFrequency > 5
              ? "low"
              : "stale",
    };
  }
   ...
   */
  async analyzeBreakingChanges(packageName, currentVersion, latestVersion) {
   ...
   * Analyze bundle
   */
  async analyzeBundle(packageName, currentVersion, latestVersion) {
    try {
   ...
   * Analyze license
   */
  async analyzeLicense(packageName, currentVersion, latestVersion) {
    ...
    if (!compatible) {
      issues.push(
        `License ${latestLicense} may not be compatible with MIT projects`,
      );
    }
    ...
    if (params.security.critical > 0) {
      recommendations.push({
        ...
        reasoning: [
          ...
          params.security.fixAvailable
            ? `Fix available in ${params.security.fixedInVersion}`
    ...
    if (params.currentVersion !== params.latestVersion) {
      ...
      recommendations.push({
        ...
        reasoning: [
          ...
          params.bundle
            ? `Bundle size ${params.bundle.impact.trend} by ${params.bundle.impact.percentage.toFixed(1)}%`
            : "",
          params.maintenance.isActive
            ? "Package is actively maintained"
            : "Package maintenance is stale",
        ].filter(Boolean),
        action: isMinor
          ? "Safe to upgrade"
   ...
   * Calculate risk assessment
   */
  calculateRiskAssessment(params) {
    const factors = [];
    let score = 0;
    // Security factors
    if (params.security.critical > 0) {
  ...
  createUpgradePlan(params) {
    ...
    params.breaking.recommendedPath.forEach((version, index) => {
      ...
      steps.push({
        ...
        estimatedTime:
          versionBreaking.length > 5 ? "2-4 hours" : "30-60 minutes",
  ...
  createRollbackStrategy(params) {
    ...
    if (hasLockfile) {
      ...
      steps.push(
        `Run ${params.context.packageManager} install ${params.packageName}@${params.currentVersion}`,
      );
    ...
    return {
      ...
      notes: [
        hasLockfile
          ? "Lockfile makes rollback straightforward"
          : "No lockfile - may need to manually verify versions",
