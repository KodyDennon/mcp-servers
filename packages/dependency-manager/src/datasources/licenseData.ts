...
export class LicenseDataClient {
  private licenseCategories: Record<string, string[]>;
  ...
  constructor() {
    ...
    this.licenseCategories = {
      ...
      copyleft: ["GPL-2.0", "GPL-2.0-only", "GPL-3.0", "GPL-3.0-only", "AGPL-3.0", "AGPL-3.0-only"],
      "weak-copyleft": [
        "LGPL-2.1",
        "LGPL-2.1-only",
        "LGPL-3.0",
  ...
  getLicenseInfo(licenseId: string): LicenseInfo | null {
    ...
    try {
      ...
      const spdxLicense = (spdxLicenses as any)[normalized];
  ...
  isCompatible(license1: string, license2: string): boolean {
    try {
      ...
      logger.debug("License compatibility check failed", { license1, license2, error });
      return false;
    }
  }
   ...
   */
  checkCompatibility(licenses: string[], projectLicense?: string): LicenseCompatibility[] {
    const correlationId = Logger.generateCorrelationId();
        ...
        for (let j = i + 1; j < licenses.length; j++) {
          ...
          if (!compatible) {
            results.push({
              ...
              warnings: [`${licenses[i]} and ${licenses[j]} may not be compatible`],
            });
          }
        }
      }
  ...
  validateLicense(licenseId: string): {
    ...
    const allowed = licenseConfig.allowlist.length === 0 || licenseConfig.allowlist.includes(normalized);
   ...
   */
  getLicensesByCategory(category: "permissive" | "copyleft" | "weak-copyleft" | "proprietary"): string[] {
   ...
   * Normalize license ID
   */
  private normalizeLicenseId(licenseId: string): string {
