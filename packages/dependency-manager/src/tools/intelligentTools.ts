...
export async function analyzePackageIntelligent(params: {
  ...
  try {
    ...
    const currentVersion = params.currentVersion || await npmClient.getLatestVersion(params.package);
    ...
    output.push(`\n## Security Analysis`);
    output.push(`- **Vulnerabilities:** ${analysis.analysis.security.vulnerabilities}`);
    ...
    output.push(`- **Known Exploits:** ${analysis.analysis.security.hasKnownExploits ? 'Yes ⚠️' : 'No'}`);
    output.push(`- **Fix Available:** ${analysis.analysis.security.fixAvailable ? `Yes (${analysis.analysis.security.fixedInVersion})` : 'No'}`);

    if (analysis.analysis.security.oldestVulnerability) {
      output.push(`- **Oldest Vulnerability:** ${analysis.analysis.security.oldestVulnerability.id} (${analysis.analysis.security.oldestVulnerability.ageInDays} days old)`);
    }

    output.push(`\n## Quality Analysis`);
    output.push(`- **Score:** ${analysis.analysis.quality.score.toFixed(0)}/100`);
    output.push(`- **Has Tests:** ${analysis.analysis.quality.hasTests ? 'Yes' : 'No'}`);
    output.push(`- **Has CI:** ${analysis.analysis.quality.hasCI ? 'Yes' : 'No'}`);
    output.push(`- **Has Documentation:** ${analysis.analysis.quality.hasDocumentation ? 'Yes' : 'No'}`);
    output.push(`- **Has Types:** ${analysis.analysis.quality.hasTypes ? 'Yes' : 'No'}`);
    output.push(`- **Popularity:**`);
    output.push(`  - Downloads: ${analysis.analysis.quality.popularity.downloads.toLocaleString()}`);
    output.push(`  - Stars: ${analysis.analysis.quality.popularity.stars.toLocaleString()}`);
    output.push(`  - Dependents: ${analysis.analysis.quality.popularity.dependents.toLocaleString()}`);

    output.push(`\n## Maintenance Analysis`);
    output.push(`- **Active:** ${analysis.analysis.maintenance.isActive ? 'Yes' : 'No'}`);
    output.push(`- **Deprecated:** ${analysis.analysis.maintenance.isDeprecated ? 'Yes ⚠️' : 'No'}`);
    output.push(`- **Last Publish:** ${analysis.analysis.maintenance.lastPublish} (${analysis.analysis.maintenance.daysSinceLastPublish} days ago)`);
    output.push(`- **Release Frequency:** ${analysis.analysis.maintenance.releaseFrequency}`);
    output.push(`- **Maintainer Count:** ${analysis.analysis.maintenance.maintainerCount}`);
    output.push(`- **Commit Activity:** ${analysis.analysis.maintenance.commitActivity}`);

    if (analysis.analysis.bundle) {
      output.push(`\n## Bundle Analysis`);
      output.push(`- **Size:** ${(analysis.analysis.bundle.size / 1024).toFixed(2)} KB`);
      output.push(`- **Gzipped:** ${(analysis.analysis.bundle.gzip / 1024).toFixed(2)} KB`);
      output.push(`- **Impact:** ${analysis.analysis.bundle.impact.trend} by ${analysis.analysis.bundle.impact.percentage.toFixed(1)}%`);
      output.push(`- **Tree Shakeable:** ${analysis.analysis.bundle.treeShakleable ? 'Yes' : 'No'}`);
      output.push(`- **Has Side Effects:** ${analysis.analysis.bundle.hasSideEffects ? 'Yes' : 'No'}`);
    }

    output.push(`\n## License Analysis`);
    output.push(`- **License:** ${analysis.analysis.license.license}`);
    output.push(`- **Category:** ${analysis.analysis.license.category}`);
    output.push(`- **Compatible:** ${analysis.analysis.license.compatible ? 'Yes' : 'No'}`);
    output.push(`- **Changed from Previous:** ${analysis.analysis.license.changedFromPrevious ? 'Yes' : 'No'}`);
    ...
    output.push(`\n## Risk Assessment`);
    output.push(`- **Overall Risk:** ${analysis.riskAssessment.overall.toUpperCase()}`);
    ...
    output.push(`\n## Upgrade Plan`);
    output.push(`- **Recommended:** ${analysis.upgradePlan.recommended ? 'Yes' : 'No'}`);
    output.push(`- **Can Automate:** ${analysis.upgradePlan.canAutomate ? 'Yes' : 'No'}`);
    output.push(`- **Total Estimated Time:** ${analysis.upgradePlan.totalEstimatedTime}`);
    ...
    output.push(`- **Estimated Time:** ${analysis.rollbackStrategy.estimatedTime}`);
    output.push(`- **Data Loss:** ${analysis.rollbackStrategy.dataLoss ? 'Yes' : 'No'}`);

    output.push(`\n## Project Context`);
    output.push(`- **Project Type:** ${analysis.context.projectType}`);
    output.push(`- **Package Manager:** ${analysis.context.packageManager}`);
    output.push(`- **Monorepo:** ${analysis.context.isMonorepo ? 'Yes' : 'No'}`);
    output.push(`- **Dependencies:** ${analysis.context.dependencies.total} total (${analysis.context.dependencies.direct} direct)`);
...
export async function findAlternativePackages(params: {
  ...
  try {
    const alternatives = await alternativeDiscovery.findAlternatives(params.package, {
    ...
    for (const alt of alternatives) {
      ...
      output.push(`- **Estimated Effort:** ${alt.migrationDifficulty.estimatedHours}`);
      output.push(`- **API Compatibility:** ${alt.migrationDifficulty.apiCompatibility}`);
      ...
      output.push(`\n### Comparison`);
      output.push(`\n**Popularity:**`);
      output.push(`- Downloads: ${alt.comparison.popularity.downloads.alternative.toLocaleString()} (${alt.comparison.popularity.downloads.winner})`);
      output.push(`- Stars: ${alt.comparison.popularity.stars.alternative.toLocaleString()} (${alt.comparison.popularity.stars.winner})`);

      output.push(`\n**Quality:**`);
      output.push(`- Score: ${alt.comparison.quality.score.alternative.toFixed(0)}/100 (${alt.comparison.quality.score.winner}, ${alt.comparison.quality.score.difference > 0 ? '+' : ''}${alt.comparison.quality.score.difference.toFixed(1)})`);
      output.push(`- TypeScript: ${alt.comparison.quality.hasTypes.alternative ? 'Yes' : 'No'} (${alt.comparison.quality.hasTypes.winner})`);

      if (alt.comparison.bundle.size.alternative > 0) {
        output.push(`\n**Bundle Size:**`);
        output.push(`- Size: ${(alt.comparison.bundle.size.alternative / 1024).toFixed(2)} KB (${alt.comparison.bundle.size.winner})`);
        output.push(`- Gzipped: ${(alt.comparison.bundle.gzip.alternative / 1024).toFixed(2)} KB (${alt.comparison.bundle.gzip.winner})`);
      }

      output.push(`\n**Security:**`);
      output.push(`- Vulnerabilities: ${alt.comparison.security.vulnerabilities.alternative} (${alt.comparison.security.vulnerabilities.winner})`);
      output.push(`- Critical: ${alt.comparison.security.critical.alternative} (${alt.comparison.security.critical.winner})`);

      output.push(`\n**License:**`);
      output.push(`- ${alt.comparison.license.alternative} (${alt.comparison.license.note})`);

      output.push(`\n**Features:**`);
      output.push(`- Similarity: ${alt.comparison.features.similarity.toFixed(0)}%`);
      output.push(`- Likely Compatible: ${alt.comparison.features.likelyCompatible ? 'Yes' : 'No'}`);
      if (alt.comparison.features.sharedKeywords.length > 0) {
        output.push(`- Shared Keywords: ${alt.comparison.features.sharedKeywords.join(', ')}`);
      }
...
export async function analyzeDependencyGraph(params: {
  ...
  try {
    ...
    output.push(`- **Direct Dependencies:** ${analysis.statistics.directDependencies}`);
    output.push(`- **Transitive Dependencies:** ${analysis.statistics.transitiveDependencies}`);
    output.push(`- **Max Depth:** ${analysis.statistics.maxDepth}`);
    output.push(`- **Average Depth:** ${analysis.statistics.averageDepth.toFixed(2)}`);
    output.push(`- **Duplicate Versions:** ${analysis.statistics.duplicateCount}`);
    output.push(`- **Circular Dependencies:** ${analysis.statistics.circularCount}`);
    output.push(`- **Peer Conflicts:** ${analysis.statistics.peerConflictCount}`);
    ...
    if (analysis.peerConflicts.length > 0) {
      ...
      analysis.peerConflicts.slice(0, 10).forEach((conflict) => {
        ...
        output.push(`- **Installed:** ${conflict.installed || 'Not installed'}`);
    ...
    if (analysis.transitiveImpacts.length > 0) {
      ...
      analysis.transitiveImpacts.slice(0, 10).forEach((impact, i) => {
        ...
        output.push(`- **Critical Path:** ${impact.criticalPath ? 'Yes' : 'No'}`);
      });
    }
    ...
    logger.info('Dependency graph analysis complete', {
      ...
      issueCount: analysis.circularDependencies.length +
...
export function getIntelligentTools() {
  return [
    ...
    },
    {
      ...
      description: 'Perform comprehensive AI-ready analysis of a package (security, quality, maintenance, bundle, license, risk assessment, recommendations)',
      inputSchema: {
        type: 'object',
        properties: {
          package: {
          ...
          projectPath: {
            type: 'string',
            description: 'Path to project directory (optional, defaults to cwd)',
          },
        },
        required: ['package'],
      },
    ...
    },
    {
      ...
      description: 'Analyze dependency graph for circular dependencies, duplicates, peer conflicts, and transitive impacts',
      inputSchema: {
        type: 'object',
        properties: {
          projectPath: {
 ...
 */
export async function handleIntelligentToolCall(
  name: string,
    ...
    switch (name) {
      ...
      case 'deps_find_alternatives':
        result = await findAlternativePackages({
          ...
          minQualityScore: args.minQualityScore as number | undefined,
          requireTypes: args.requireTypes as boolean | undefined,
          maxBundleSize: args.maxBundleSize as number | undefined,
        });
        break;
