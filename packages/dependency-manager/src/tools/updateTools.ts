...
export function getUpdateTools() {
  return [
    ...
    },
    {
      ...
      description: "Suggest alternative packages with better maintenance or performance",
      inputSchema: {
        type: "object",
        properties: {
          package_name: { type: "string", description: "Package name to find alternatives for" },
        },
...
export async function handleUpdateToolCall(name: string, args: Record<string, unknown>) {
  switch (name) {
    case "deps_check_updates": {
      ...
      try {
        ...
        for (const [pkg, currentRange] of Object.entries<string>(dependencies)) {
          try {
            const { stdout } = await execAsync(`npm view ${pkg} version --json`, { cwd: directory });
            const latest = stdout.trim().replace(/"/g, "");
            const current = currentRange.replace(/^[\^~]/, "");

            if (semver.valid(current) && semver.valid(latest) && semver.gt(latest, current)) {
              ...
              text: JSON.stringify(
                {
                  ...
                  recommendation: "These updates are unlikely to contain breaking changes",
                },
                null,
                2
              ),
    ...
    case "deps_get_changelog": {
      const { package_name, from_version, to_version } = args as {
        package_name: string;
        from_version: string;
        to_version: string;
      };
      ...
      try {
        ...
        const { stdout } = await execAsync(`npm view ${package_name} repository.url --json`);
