...
export class RateLimiter {
   ...
   */
  private createLimiter(name: string): Bottleneck {
    const rateLimitConfig = config.get("rateLimit");
    ...
    limiter.on("failed", async (error, jobInfo) => {
      ...
      if (jobInfo.retryCount < rateLimitConfig.maxRetries) {
        ...
        logger.debug("Retrying request", { limiter: name, delay, retryCount: jobInfo.retryCount });
        return delay;
      }
   ...
   */
  async enqueue<T>(fn: () => Promise<T>, options?: { priority?: number }): Promise<T> {
    if (!config.get("rateLimit").enabled) {
      return await fn();
    }
   ...
   */
  isAtCapacity(service: string = "npm-registry"): boolean {
   ...
   */
  getQueueStatus(service: string = "npm-registry"): {
    executing: number;
    queued: number;
    done: number;
  } {
   ...
   */
  private getStats(service: string): {
    requests: number;
    failures: number;
    retries: number;
  } {
   ...
   * Get all statistics
   */
  getAllStats(): Map<string, { requests: number; failures: number; retries: number }> {
