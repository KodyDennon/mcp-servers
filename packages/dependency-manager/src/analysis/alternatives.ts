...
export interface Alternative {
  ...
  version: string;
  description: string;
  score: number; // 0-100, how good an alternative it is
...
export class AlternativeDiscovery {
  private static instance: AlternativeDiscovery;
      ...
      async () => {
        ...
        return await cache.getOrSet(
          ...
          async () => {
            ...
            const originalMetadata = await npmClient.getPackageMetadata(packageName);
            ...
            for (const candidate of filtered.slice(0, options.maxResults || 10)) {
  ...
  private async searchSimilarPackages(
    ...
    const nameComponents = packageName.split(/[-_@/]/).filter((c) => c.length > 3);
      ...
      try {
        ...
        if (options.maxBundleSize) {
          try {
            ...
            const bundleInfo = await bundleClient.getBundleSize(candidate, latestVersion);
  ...
  async comparePackages(
    ...
    return await logger.time(
      ...
      async () => {
        ...
        const alternativeVersion = await npmClient.getLatestVersion(alternativeName);
      ...
      { correlationId, operation: 'comparePackages', originalName, alternativeName }
    );
  }
  ...
  private comparePopularity(
    originalQuality: any,
    alternativeQuality: any,
    originalMeta: any,
    alternativeMeta: any
  ): PopularityComparison {
    const originalDownloads = originalQuality.popularity.downloadsCount || 0;
    const alternativeDownloads = alternativeQuality.popularity.downloadsCount || 0;
    ...
    const alternativeDependents = alternativeQuality.popularity.dependentsCount || 0;
  ...
  private compareMaintenance(
    originalMeta: any,
    alternativeMeta: any,
    originalMaintenance: any,
    alternativeMaintenance: any
    ...
    const originalDate = originalLastPublish ? new Date(originalLastPublish).getTime() : 0;
    const alternativeDate = alternativeLastPublish ? new Date(alternativeLastPublish).getTime() : 0;

    return {
      lastPublish: {
        original: originalLastPublish,
        alternative: alternativeLastPublish,
        winner: alternativeDate > originalDate ? 'alternative' :
                originalDate > alternativeDate ? 'original' : 'tie',
      },
      releaseFrequency: {
        original: this.getReleaseFrequency(originalMaintenance.releasesCount || 0),
        alternative: this.getReleaseFrequency(alternativeMaintenance.releasesCount || 0),
        winner: (alternativeMaintenance.releasesCount || 0) > (originalMaintenance.releasesCount || 0)
          ? 'alternative'
          : (originalMaintenance.releasesCount || 0) > (alternativeMaintenance.releasesCount || 0)
          ? 'original'
          : 'tie',
      },
      isDeprecated: {
        ...
        winner: originalMeta.deprecated && !alternativeMeta.deprecated ? 'alternative' :
                !originalMeta.deprecated && alternativeMeta.deprecated ? 'original' : 'tie',
      },
    };
  }
  ...
  private compareQuality(
    ...
    const alternativeHasTypes = !!(alternativeMeta.types || alternativeMeta.typings);
    ...
    return {
      score: {
        ...
        winner: this.determineWinner(alternativeScore / (originalScore || 1), 0.95, 1.05),
        difference: scoreDiff,
      },
      hasTypes: {
        original: originalHasTypes,
        alternative: alternativeHasTypes,
        winner: alternativeHasTypes && !originalHasTypes ? 'alternative' :
                originalHasTypes && !alternativeHasTypes ? 'original' : 'tie',
      },
      hasTests: {
        original: originalHasTests,
        alternative: alternativeHasTests,
        winner: alternativeHasTests && !originalHasTests ? 'alternative' :
                originalHasTests && !alternativeHasTests ? 'original' : 'tie',
      },
    };
  }
    ...
    try {
      ...
      return {
        size: {
          ...
          winner: alternativeBundle.size < originalBundle.size * 0.95 ? 'alternative' :
                  originalBundle.size < alternativeBundle.size * 0.95 ? 'original' : 'tie',
          difference: sizeDiff,
        ...
        gzip: {
          ...
          winner: alternativeBundle.gzip < originalBundle.gzip * 0.95 ? 'alternative' :
                  originalBundle.gzip < alternativeBundle.gzip * 0.95 ? 'original' : 'tie',
          difference: gzipDiff,
  ...
  private async compareSecurity(
    ...
    try {
      const [originalSecurity, alternativeSecurity] = await Promise.all([
        ...
        securityClient.getVulnerabilityReport(alternativeName, alternativeVersion),
      ]);
      ...
      return {
        vulnerabilities: {
          ...
          winner: alternativeSecurity.summary.total < originalSecurity.summary.total ? 'alternative' :
                  originalSecurity.summary.total < alternativeSecurity.summary.total ? 'original' : 'tie',
        },
        critical: {
          original: originalSecurity.summary.critical,
          alternative: alternativeSecurity.summary.critical,
          winner: alternativeSecurity.summary.critical < originalSecurity.summary.critical ? 'alternative' :
                  originalSecurity.summary.critical < alternativeSecurity.summary.critical ? 'original' : 'tie',
        },
      };
    } catch (error) {
  ...
  private compareLicenses(
    ...
    const compatible = licenseClient.isCompatible(originalLicense, alternativeLicense);
  ...
  private compareFeatures(
    ...
    const uniqueToOriginal: string[] = [];
    const uniqueToAlternative: string[] = [];
    ...
    const similarity = totalKeywords > 0
  ...
  private calculateMigrationDifficulty(
    ...
    const nameSimilarity = this.calculateNameSimilarity(originalName, alternativeName);
    let apiCompatibility: MigrationDifficulty['apiCompatibility'] = 'unknown';
    ...
    let level: MigrationDifficulty['level'] = 'easy';
   ...
   */
  private calculateAlternativeScore(params: {
    popularity: PopularityComparison;
    maintenance: MaintenanceComparison;
    quality: QualityComparison;
    bundle: BundleComparison;
    ...
    if (params.security.critical.alternative < params.security.critical.original) {
      score += 15;
    } else if (params.security.critical.alternative > params.security.critical.original) {
      score -= 15;
    }
    ...
    if (params.maintenance.isDeprecated.alternative && !params.maintenance.isDeprecated.original) {
      score -= 20; // Big penalty for deprecated
    } else if (!params.maintenance.isDeprecated.alternative && params.maintenance.isDeprecated.original) {
   ...
   */
  private generateProsConsReasoning(params: {
    popularity: PopularityComparison;
    maintenance: MaintenanceComparison;
    quality: QualityComparison;
    bundle: BundleComparison;
    ...
    features: FeatureComparison;
    migration: MigrationDifficulty;
  }): { pros: string[]; cons: string[]; reasoning: string[] } {
    const pros: string[] = [];
    const cons: string[] = [];
    ...
    if (params.quality.score.winner === 'alternative') {
      pros.push(`Higher quality score (+${params.quality.score.difference.toFixed(1)} points)`);
      reasoning.push('Better overall package quality');
    } else if (params.quality.score.winner === 'original') {
      cons.push(`Lower quality score (${params.quality.score.difference.toFixed(1)} points)`);
    }
    ...
    if (params.bundle.gzip.winner === 'alternative') {
      pros.push(`Smaller bundle (${params.bundle.gzip.differencePercent.toFixed(1)}% reduction)`);
      reasoning.push('Better performance impact');
    } else if (params.bundle.gzip.winner === 'original') {
      cons.push(`Larger bundle (+${params.bundle.gzip.differencePercent.toFixed(1)}%)`);
    }
    ...
    if (params.features.similarity > 60) {
      pros.push(`High feature similarity (${params.features.similarity.toFixed(0)}%)`);
    ...
    // Migration difficulty
    if (params.migration.level === 'trivial' || params.migration.level === 'easy') {
      pros.push(`Easy migration (${params.migration.estimatedHours})`);
    } else if (params.migration.level === 'hard' || params.migration.level === 'very-hard') {
    ...
    } else if (params.maintenance.isDeprecated.original && !params.maintenance.isDeprecated.alternative) {
   ...
   */
  private getReleaseFrequency(releasesPerYear: number): string {
