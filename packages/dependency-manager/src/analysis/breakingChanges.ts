...
import { ErrorRecovery, NetworkError, PackageNotFoundError } from '../errors/errors.js';
...
export interface BreakingChange {
  ...
  version: string;
  introduced: string; // version where introduced
}
...
export interface VersionComparison {
  ...
  recommendedPath: string[]; // versions to upgrade through
}
...
interface ChangelogEntry {
  ...
  date?: string;
  changes: {
    type: 'breaking' | 'feature' | 'fix' | 'deprecation' | 'other';
    description: string;
  }[];
}
...
export class BreakingChangeDetector {
  private static instance: BreakingChangeDetector;
   ...
   */
  private constructChangelogUrl(repoUrl: string): string | null {
   ...
   */
  private async fetchChangelog(url: string): Promise<string | null> {
    const changelogNames = ['CHANGELOG.md', 'HISTORY.md', 'RELEASES.md', 'CHANGES.md'];
    const branches = ['main', 'master'];

    for (const branch of branches) {
      for (const name of changelogNames) {
        try {
          const testUrl = url.replace(/\/[^/]+$/, `/${name}`).replace(/\/(main|master)\//, `/${branch}/`);

          const response = await ErrorRecovery.withRetry(
            async () => {
   ...
   */
  private parseChangelog(changelog: string, version: string): BreakingChange[] {
    const breakingChanges: BreakingChange[] = [];
   ...
   */
  private async analyzeReleaseNotes(
    packageName: string,
    versions: string[]
  ): Promise<BreakingChange[]> {
    try {
      ...
      for (const release of releases) {
        ...
        const releaseBreaking = this.parseReleaseNotes(release.body, releaseVersion);
        breakingChanges.push(...releaseBreaking);
      }
   ...
   */
  private parseReleaseNotes(body: string, version: string): BreakingChange[] {
    const breakingChanges: BreakingChange[] = [];
  ...
  private async detectAPIChanges(
    ...
    try {
      ...
      const fromMeta = await npmClient.getVersionMetadata(packageName, fromVersion);
   ...
   */
  private async extractMigrationGuides(
    packageName: string,
    fromVersion: string,
    toVersion: string,
   ...
   */
  private mapPatternTypeToBreakingType(patternType: string): BreakingType {
    const mapping: Record<string, BreakingType> = {
      removed: 'api-removal',
      renamed: 'api-change',
   ...
   */
  private inferSeverity(content: string, type: string): BreakingSeverity {
   ...
   */
  private estimateMigrationEffort(stepCount: number): 'low' | 'medium' | 'high' | 'critical' {
