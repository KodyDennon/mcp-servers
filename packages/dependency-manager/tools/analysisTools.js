...
export function getDependencyAnalysisTools() {
  return [
    ...
    },
    {
      ...
      description:
        "List all outdated dependencies with current and latest versions",
      inputSchema: {
        type: "object",
        properties: {
          directory: { type: "string", description: "Project directory path" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          package_name: {
            type: "string",
            description: "Specific package to analyze (optional)",
          },
        },
        required: ["directory"],
      },
...
export async function handleAnalysisToolCall(name, args) {
 ...
 */
async function analyzePackage(directory, correlationId) {
 ...
 */
async function listOutdated(directory, correlationId) {
          ...
          return await ErrorRecovery.withRetry(
            async () => {
              return await rateLimiter.execute(async () => {
                ...
                const formatted = Object.entries(outdated).map(
                  ([name, info]) => ({
                    package: name,
                  ...
                  content: [
                    {
                      type: "text",
                      text: JSON.stringify(
                        { outdated: formatted, total: formatted.length },
                        null,
                        2,
                      ),
                    },
 ...
 * Analyze bundle size
 */
async function analyzeBundleSize(directory, args, correlationId) {
      ...
      return await logger.time(
        ...
        async () => {
          ...
          try {
            ...
            throw new FileSystemError(
              `Failed to analyze bundle size: ${error.message}`,
              error,
            );
          }
        },
 ...
 */
async function findDuplicates(directory, correlationId) {
              ...
              content: [
                {
                  type: "text",
                  text: JSON.stringify(
                    { duplicates, total: duplicates.length },
                    null,
                    2,
                  ),
                },
