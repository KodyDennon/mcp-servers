...
export function getSecurityTools() {
  return [
    ...
    },
    {
      ...
      description:
        "Check security status of a specific package using npm registry",
      inputSchema: {
        type: "object",
        properties: {
          package_name: {
...
export async function handleSecurityToolCall(name, args) {
 ...
 */
async function auditVulnerabilities(args, correlationId) {
  const { directory, level } = args;
        ...
        async () => {
          try {
            ...
            const summary = {
              ...
              advisories: Object.entries(audit.advisories || {}).map(
                ([id, adv]) => ({
                  id,
            ...
            logger.info("Vulnerabilities audited", {
              correlationId,
              total: Object.values(summary.vulnerabilities).reduce(
                (a, b) => a + b,
                0,
              ),
            });
            ...
            throw new SecurityAuditError(
              `Failed to run security audit: ${error.message}`,
              error,
            );
          }
        },
 ...
 */
async function getSecurityAdvisories(args, correlationId) {
      ...
      return await logger.time(
        ...
        async () => {
          try {
            ...
            const audit = JSON.parse(stdout);
            const advisories = Object.entries(audit.advisories || {}).map(
              ([id, adv]) => ({
                id,
                severity: adv.severity,
                title: adv.title,
              ...
              content: [
                {
                  type: "text",
                  text: JSON.stringify(
                    { advisories, total: advisories.length },
                    null,
                    2,
                  ),
                },
            ...
            throw new SecurityAuditError(
              `Failed to get security advisories: ${error.message}`,
              error,
            );
          }
        },
 ...
 */
async function checkPackageSecurity(args, correlationId) {
              ...
              return await rateLimiter.execute(async () => {
                try {
                  ...
                  throw new NetworkError(
                    `Failed to check package security: ${error}`,
                    error,
                  );
                }
              }, "npm-registry");
