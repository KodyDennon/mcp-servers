...
export class PackageQualityClient {
        ...
        return await logger.time(
          ...
          async () => {
            return await rateLimiter.execute(async () => {
              try {
                const response = await this.npmsClient.get(
                  `/package/${encodeURIComponent(packageName)}`,
                );
                const data = response.data;
                const overall = data.score?.final
                  ? Math.round(data.score.final * 100)
                  : 0;
                ...
                return {
                  ...
                  popularity: {
                    ...
                    downloadsCount:
                      data.collected?.npm?.downloads?.[0]?.count || 0,
                    downloadsAcceleration:
                      data.evaluation?.popularity?.downloadsAcceleration || 0,
                    dependentsCount: data.collected?.npm?.dependentsCount || 0,
                  },
                  maintenance: {
                    score: data.score?.detail?.maintenance || 0,
                    releasesFrequency:
                      data.evaluation?.maintenance?.releasesFrequency || 0,
                    commitsFrequency:
                      data.evaluation?.maintenance?.commitsFrequency || 0,
                    openIssues: data.collected?.github?.issues?.openCount || 0,
                    issuesDistribution:
                      data.evaluation?.maintenance?.issuesDistribution || 0,
                  },
                };
              } catch (error) {
   ...
   */
  async getDependencyHealth(packageName, version, options) {
    const correlationId = Logger.generateCorrelationId();
    return await logger.time(
      ...
      async () => {
        ...
        if (maintenance.releaseFrequency === "abandoned") {
          score -= 30;
          issues.push(
            `Not updated in ${maintenance.daysSinceLastPublish} days`,
          );
   ...
   */
  async comparePackages(packages) {
    const correlationId = Logger.generateCorrelationId();
    return await logger.time(
      "comparePackages",
      async () => {
   ...
   */
  async getPackageTrends(packageName) {
    const correlationId = Logger.generateCorrelationId();
    return await logger.time(
      `getPackageTrends:${packageName}`,
      async () => {
        const [dayDownloads, weekDownloads, monthDownloads, metadata] =
          await Promise.all([
        ...
        const recentVersions = Object.entries(metadata.time || {}).filter(
          ([version, time]) => {
...
//# sourceMappingURL=packageQuality.js.map
