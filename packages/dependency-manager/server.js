  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name.startsWith("deps_analyze_") || name.startsWith("deps_list_")) {
        const { handleAnalysisToolCall } = await import(
      ...
      if (name.startsWith("deps_security_") || name.startsWith("deps_audit_")) {
        const { handleSecurityToolCall } = await import(
          "./tools/securityTools.js"
        );
        return await handleSecurityToolCall(name, args || {});
      }
      // License tools
      if (name.startsWith("deps_license_")) {
        const { handleLicenseToolCall } = await import(
          "./tools/licenseTools.js"
        );
      ...
      if (
        name.startsWith("deps_analyze_breaking") ||
