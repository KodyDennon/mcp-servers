  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          status: {
            type: "string",
            description: "Status filter: unresolved, resolved, ignored",
          },
          limit: {
            type: "number",
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          stat: {
            type: "string",
            description: "Stat to retrieve: received, rejected, blacklisted",
          },
...
export async function handleSentryToolCall(name, args) {
  if (!process.env.SENTRY_AUTH_TOKEN || !process.env.SENTRY_ORG) {
    throw new Error(
      "SENTRY_AUTH_TOKEN and SENTRY_ORG environment variables are required",
    );
  }
  const org = process.env.SENTRY_ORG;
  switch (name) {
    case "sentry_list_issues": {
      const { project_slug, query, status = "unresolved", limit = 25 } = args;
      const params = { query: status, limit };
    ...
    case "sentry_update_issue": {
      const { issue_id, status, assignedTo } = args;
    ...
    case "sentry_get_stats": {
      const { project_slug, stat = "received", since, until } = args;
      const params = { stat };
      if (since) params.since = since;
      if (until) params.until = until;
      const response = await sentryApi.get(
        `/projects/${org}/${project_slug}/stats/`,
        { params },
      );
      return {
        content: [
...
//# sourceMappingURL=sentryTools.js.map
