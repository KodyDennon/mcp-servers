  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          status: { type: "string", description: "Status filter: active, resolved, muted" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          item_id: { type: "string", description: "Item ID" },
          status: { type: "string", description: "New status: active, resolved, muted" },
        },
...
export async function handleRollbarToolCall(name: string, args: Record<string, unknown>) {
  ...
  switch (name) {
    case "rollbar_list_items": {
      const { project_id, status = "active", limit = 20 } = args as {
        project_id: string;
        status?: string;
        limit?: number;
      };
