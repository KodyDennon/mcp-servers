...
export async function handleLogRocketToolCall(name: string, args: Record<string, unknown>) {
  ...
  switch (name) {
    case "logrocket_list_sessions": {
      const { app_slug, limit = 20 } = args as { app_slug: string; limit?: number };
    ...
    case "logrocket_get_session": {
      const { session_url } = args as { session_url: string };
