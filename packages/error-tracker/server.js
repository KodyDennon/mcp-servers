    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (name.startsWith("datadog_")) {
                const { handleDatadogToolCall } = await import("./tools/datadogTools.js");
                return await handleDatadogToolCall(name, args || {});
            }
            // New Relic tools
            if (name.startsWith("newrelic_")) {
                const { handleNewRelicToolCall } = await import("./tools/newrelicTools.js");
                return await handleNewRelicToolCall(name, args || {});
            }
            // LogRocket tools
            if (name.startsWith("logrocket_")) {
                const { handleLogRocketToolCall } = await import("./tools/logrocketTools.js");
                return await handleLogRocketToolCall(name, args || {});
            }
            // Rollbar tools
            if (name.startsWith("rollbar_")) {
                const { handleRollbarToolCall } = await import("./tools/rollbarTools.js");
