...
jest.mock('../src/utils/directoryLoader', () => ({
    DirectoryLoader: jest.fn().mockImplementation(() => ({
        load: jest.fn().mockResolvedValue([{ pageContent: 'mock directory content' }]),
    })),
}));
...
describe('aiTools', () => {
    ...
    beforeEach(() => {
        ...
        mockSupabase = {
            rpc: jest.fn().mockResolvedValue({ data: [{ content: 'test content' }], error: null }),
        ...
        currentMockOpenAIInstance.embeddings.create.mockResolvedValue({ data: [{ embedding: [0.1, 0.2] }] });
        currentMockOpenAIInstance.chat.completions.create.mockResolvedValue({ choices: [{ message: { content: 'test response' } }] });
    ...
    describe('rag', () => {
        it('should perform RAG with automatic context selection', async () => {
            ...
            mockSupabase.rpc
                ...
                .mockResolvedValueOnce({ data: { columns: [{ name: 'content', type: 'text' }] }, error: null }) // getTableSchema
                .mockResolvedValueOnce({ data: [{ content: 'test content' }], error: null }); // match_documents

            const result = await rag.execute({ query: 'test query', textColumn: 'content', vectorColumn: 'embedding' });

            expect(mockSupabase.rpc).toHaveBeenCalledWith('listTables');
            expect(mockSupabase.rpc).toHaveBeenCalledWith('getTableSchema', { tableName: 'test_table' });
            expect(currentMockOpenAIInstance.embeddings.create).toHaveBeenCalledTimes(1); // For query embedding
            expect(currentMockOpenAIInstance.chat.completions.create).toHaveBeenCalledTimes(2); // For table selection and final response
        ...
        it('should throw an error if OPENAI_API_KEY is not set', async () => {
            ...
            await expect(rag.execute({ query: 'test query', textColumn: 'content', vectorColumn: 'embedding' }))
    ...
    describe('indexDirectory', () => {
        it('should index a directory', async () => {
            ...
            expect(chunkText).toHaveBeenCalledWith('mock directory content', 1000, 200);
            expect(currentMockOpenAIInstance.embeddings.create).toHaveBeenCalledTimes(1);
        ...
        it('should throw an error if OPENAI_API_KEY is not set', async () => {
            ...
            await expect(indexDirectory.execute({
                directoryPath: '/test/dir',
                tableName: 'test_table',
                textColumn: 'content',
                vectorColumn: 'embedding',
    ...
    describe('indexUrl', () => {
        it('should index a URL', async () => {
            ...
            expect(CheerioWebBaseLoader.mock.results[0].value.load).toHaveBeenCalled();
