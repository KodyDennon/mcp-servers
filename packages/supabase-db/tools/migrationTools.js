...
export const runMigrationTool = {
  ...
  input_schema: z.object({
    filename: z
      .string()
      .describe(
        "Migration filename (relative to packages/db/supabase/migrations/)",
      ),
  }),
...
export const generateMigrationTool = {
  ...
  description:
    "Generate a new migration file based on the differences between the local and remote schemas.",
  input_schema: z.object({
    source: z
      .string()
...
export const seedDataTool = {
  name: "seedData",
  description:
    "Seed the database using a pre-defined seed file (e.g., supabase/seed.sql).",
  input_schema: z.object({}),
  output_schema: z.object({
    message: z.string(),
  }),
};
export async function handleMigrationToolCall(
  toolName,
  input,
  connectionManager,
) {
...
`;
        ...
        for (const tableName in diff.tablesChanged) {
          ...
          for (const col of changes.columnsAdded) {
            const localCol = localSchema[tableName].columns.find(
              (c) => c.name === col,
            );
