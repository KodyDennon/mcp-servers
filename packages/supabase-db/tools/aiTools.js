...
async function ragTool(query, textColumn, vectorColumn, embeddingModel = "text-embedding-ada-002", languageModel = "gpt-4") {
    ...
    const schemaPromises = tables.map((table) => supabase.rpc("getTableSchema", { tableName: table.name }));
    ...
    const contextTable = completionResponseForTable.choices[0].message.content.trim();
    ...
    const completionResponse = await openai.chat.completions.create({
        ...
        messages: [
            {
                role: "system",
                content: "You are a helpful assistant. Use the following context to answer the question. If you don't know the answer, say that you don't know.\n\n" +
                    `Context:\n${context}`,
            },
            {
                role: "user",
...
async function processAndEmbed(documents, tableName, textColumn, vectorColumn, embeddingModel) {
    const openai = getOpenAIClient();
    const supabase = getSupabaseClient();
    const parsedChunkSize = Number.parseInt(process.env.RAG_CHUNK_SIZE ?? "1000", 10);
    const parsedChunkOverlap = Number.parseInt(process.env.RAG_CHUNK_OVERLAP ?? "200", 10);
...
async function indexDirectoryTool(directoryPath, tableName, textColumn, vectorColumn, embeddingModel = "text-embedding-ada-002") {
    const loader = new DirectoryLoader(directoryPath);
    const docs = await loader.load();
    return processAndEmbed(docs, tableName, textColumn, vectorColumn, embeddingModel);
}
async function indexUrlTool(url, tableName, textColumn, vectorColumn, embeddingModel = "text-embedding-ada-002") {
...
export const rag = {
    name: "rag",
    description: "Performs Retrieval-Augmented Generation (RAG) directly against your Supabase data, automatically selecting the most relevant table.",
    parameters: {
        type: "object",
        properties: {
            query: {
    ...
    execute: async ({ query, textColumn, vectorColumn, embeddingModel, languageModel, }) => {
        return ragTool(query, textColumn, vectorColumn, embeddingModel, languageModel);
    },
};
export const indexDirectory = {
    name: "indexDirectory",
    description: "Scans a directory, chunks the documents, generates embeddings, and stores everything in the specified table.",
    parameters: {
        type: "object",
        properties: {
            directoryPath: {
    ...
    execute: async ({ directoryPath, tableName, textColumn, vectorColumn, embeddingModel, }) => {
        return indexDirectoryTool(directoryPath, tableName, textColumn, vectorColumn, embeddingModel);
    },
};
export const indexUrl = {
    name: "indexUrl",
    description: "Fetches content from a URL, chunks it, generates embeddings, and stores it.",
    parameters: {
        type: "object",
        properties: {
            url: {
    ...
    execute: async ({ url, tableName, textColumn, vectorColumn, embeddingModel, }) => {
        return indexUrlTool(url, tableName, textColumn, vectorColumn, embeddingModel);
    },
};
