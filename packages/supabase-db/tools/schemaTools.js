...
export const getTableSchemaTool = {
  name: "getTableSchema",
  description:
    "Retrieves the schema (columns, types, constraints) for a specific table in the active database connection.",
  input_schema: z.object({
...
export const listIndexesTool = {
  name: "listIndexes",
  description:
    "Lists all indexes for a given table in the active database connection.",
  input_schema: z.object({
    tableName: z.string().describe("The name of the table."),
  }),
  output_schema: z.object({
...
export const listFunctionsTool = {
  name: "listFunctions",
  description:
    "Lists all stored procedures and functions in the active database connection.",
  input_schema: z.object({}),
  output_schema: z.object({
    functions: z
      .array(
...
export const searchSchemaTool = {
  name: "searchSchema",
  description:
    "Searches the database schema for tables, columns, or functions matching a keyword.",
  input_schema: z.object({
...
export const createTableTool = {
  name: "createTable",
  description:
    "Creates a new table in the database. Provide the table name and column definitions.",
  input_schema: z.object({
      ...
      .array(
        z.object({
          ...
          type: z
            .string()
            .describe(
              "Column data type (e.g., TEXT, INTEGER, UUID, BOOLEAN, TIMESTAMP WITH TIME ZONE).",
            ),
          isPrimaryKey: z
            .boolean()
            .optional()
            .describe("True if this column is part of the primary key."),
          isUnique: z
            .boolean()
            .optional()
            .describe("True if this column must have unique values."),
          isNullable: z
            .boolean()
            .optional()
            .describe("True if this column can be NULL. Defaults to false."),
          defaultValue: z
            .string()
            .optional()
            .describe(
              "Default value for the column (e.g., 'uuid_generate_v4()', 'NOW()').",
            ),
        }),
      )
      .describe("Array of column definitions."),
    primaryKey: z
      ...
      .optional()
      .describe(
        "Optional: Array of column names that form the primary key. If not provided, the first column marked as isPrimaryKey will be used.",
      ),
  }),
  output_schema: z.object({
...
export const dropTableTool = {
  name: "dropTable",
  description:
    "Drops an existing table from the database. This action is irreversible.",
  input_schema: z.object({
    ...
    cascade: z
      ...
      .optional()
      .describe(
        "Optional: Automatically drop objects that depend on the table (e.g., views). Defaults to false.",
      ),
  }),
  output_schema: z.object({
    message: z.string().describe("Confirmation message."),
...
export const addColumnTool = {
  ...
  input_schema: z.object({
    ...
    columnType: z
      .string()
      .describe(
        "The data type of the new column (e.g., TEXT, INTEGER, UUID, BOOLEAN, TIMESTAMP WITH TIME ZONE).",
      ),
    isNullable: z
      .boolean()
      .optional()
      .describe("True if this column can be NULL. Defaults to true."),
    defaultValue: z
      .string()
      .optional()
      .describe(
...
export const dropColumnTool = {
  name: "dropColumn",
  description:
    "Drops a column from an existing table. This action is irreversible.",
  input_schema: z.object({
    tableName: z.string().describe("The name of the table."),
    ...
    cascade: z
      ...
      .optional()
      .describe(
        "Optional: Automatically drop objects that depend on the column. Defaults to false.",
      ),
  }),
  output_schema: z.object({
    message: z.string().describe("Confirmation message."),
...
export const createIndexTool = {
  name: "createIndex",
  description:
    "Creates a new index on one or more columns of a table to improve query performance.",
  input_schema: z.object({
    tableName: z.string().describe("The name of the table."),
    columns: z
      .array(z.string())
      .describe("An array of column names to include in the index."),
    indexName: z
      .string()
      .optional()
      .describe(
        "Optional: A name for the index. If not provided, one will be generated.",
      ),
    isUnique: z
      .boolean()
      .optional()
      .describe("True to create a unique index. Defaults to false."),
  }),
  output_schema: z.object({
    message: z.string().describe("Confirmation message."),
  }),
...
export const diffSchemaTool = {
  name: "diffSchema",
  description:
    "Compare the schema of the local repository with the remote database.",
  input_schema: z.object({
    source: z
      .string()
      .describe("The source of the local schema (e.g., 'supabase/migrations')")
  ...
  try {
    switch (toolName) {
      ...
      case listIndexesTool.name: {
        const { tableName } = input;
        const { rows } = await client.query(
          `SELECT indexname AS name, indexdef AS definition FROM pg_indexes WHERE tablename = $1;`,
          [tableName],
        );
        return { indexes: rows };
      }
      ...
      case searchSchemaTool.name: {
        ...
        tableRows.forEach((row) =>
          searchResults.push({ type: "table", name: row.table_name }),
        );
        // Search columns
        const { rows: columnRows } = await client.query(
          `SELECT table_name, column_name, data_type FROM information_schema.columns WHERE table_schema = 'public' AND column_name ILIKE $1;`,
          [`%${keyword}%`],
        );
        ...
        functionRows.forEach((row) =>
          searchResults.push({
            type: "function",
            name: row.name,
            details: `(${row.arguments}) RETURNS ${row.returnType}`,
          }),
        );
      ...
      case addColumnTool.name: {
        ...
        if (defaultValue) colDef += ` DEFAULT ${defaultValue}`;
      ...
      case createIndexTool.name: {
        ...
        return {
          message: `Index '${name}' created on table '${tableName}' successfully.`,
        };
      }
      case diffSchemaTool.name: {
        ...
        diff.tablesAdded = localTables.filter(
          (table) => !remoteTables.includes(table),
        );
        diff.tablesRemoved = remoteTables.filter(
          (table) => !localTables.includes(table),
        );
        for (const tableName of localTables.filter((table) =>
          remoteTables.includes(table),
        )) {
          ...
          tableChanges.columnsAdded = localColumns.filter(
            (col) => !remoteColumns.includes(col),
          );
          tableChanges.columnsRemoved = remoteColumns.filter(
            (col) => !localColumns.includes(col),
          );
          for (const colName of localColumns.filter((col) =>
            remoteColumns.includes(col),
          )) {
            const localCol = localTable.columns.find((c) => c.name === colName);
            const remoteCol = remoteTable.columns.find(
              (c) => c.name === colName,
            );
            if (
          ...
          tableChanges.constraintsAdded = localConstraints.filter(
            (con) => !remoteConstraints.includes(con),
          );
          tableChanges.constraintsRemoved = remoteConstraints.filter(
            (con) => !localConstraints.includes(con),
          );
          if (
