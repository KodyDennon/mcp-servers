...
export const healthCheckTool = {
  name: "health_check",
  description:
    "Run comprehensive health diagnostics on the MCP server and database connection. Returns detailed status including database connectivity, memory usage, response times, connection pool health, and recent errors. Use this to troubleshoot issues or verify server health.",
  input_schema: {
    type: "object",
    properties: {
      include_history: {
...
export const getConnectionStatsTool = {
  ...
  description:
    "Get detailed statistics about the database connection pool including total connections, idle connections, waiting requests, query metrics, circuit breaker status, and failure rates. Useful for monitoring performance and identifying connection issues.",
  input_schema: {
    type: "object",
    properties: {
      connection_id: {
        type: "string",
        description:
          "Optional connection ID to get stats for specific connection",
      },
    },
  },
};
...
export const getRecoveryStatsTool = {
  ...
  description:
    "Get statistics about auto-recovery attempts including total attempts, successful recoveries, failed recoveries, success rate, and recent recovery history. Helps understand how well the auto-recovery system is performing.",
  input_schema: {
    type: "object",
    properties: {},
  },
...
export const resetCircuitBreakerTool = {
  ...
  description:
    "Manually reset the circuit breaker to attempt reconnection. Use this when the circuit breaker is open and you want to force a reconnection attempt without waiting for the automatic timeout. This should only be used after addressing the underlying issue causing failures.",
  input_schema: {
    type: "object",
    properties: {
      connection_id: {
 ...
 */
export async function handleMonitoringToolCall(
  toolName,
  args,
  connectionManager,
) {
    ...
    switch (toolName) {
      ...
      default:
        throw new MCPError(
          ...
          `Unknown monitoring tool: ${toolName}`,
          {},
        );
    }
  } catch (error) {
...
async function handleResetCircuitBreaker(args, connectionManager) {
  try {
    const connectionId =
      args.connection_id || connectionManager.activeConnectionId;
    ...
    if (conn.smartPool && conn.smartPool.circuitBreaker) {
      ...
      throw new MCPError(
        "VALIDATION_INVALID_INPUT",
        "Circuit breaker not available for this connection",
        {
          connection_id: connectionId,
