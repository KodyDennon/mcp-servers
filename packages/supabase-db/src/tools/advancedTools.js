...
export const getCacheStatsTool = {
  ...
  description:
    "Get query cache statistics including hit rate, size utilization, and top cached queries. Useful for monitoring cache performance.",
  input_schema: {
    type: "object",
    properties: {},
  },
...
export const clearCacheTool = {
  name: "clear_cache",
  description:
    "Clear the query cache. Optionally clear only entries matching a pattern.",
  input_schema: {
    type: "object",
    properties: {
      pattern: {
...
export const listTemplatesTool = {
  name: "list_templates",
  description:
    "List available query templates. Templates provide pre-built queries for common operations like analytics, reporting, optimization, and security audits.",
  input_schema: {
    type: "object",
    properties: {
      category: {
        type: "string",
        description:
          "Filter by category: analytics, reporting, admin, optimization, security",
      },
    },
  },
};
...
export const getTemplateTool = {
  name: "get_template",
  description:
    "Get details about a specific query template including its SQL, parameters, and description.",
  input_schema: {
    type: "object",
    properties: {
      templateId: {
...
export const executeTemplateTool = {
  ...
  description:
    "Execute a query template with the provided parameters. Templates automatically handle parameter substitution and validation.",
  input_schema: {
    type: "object",
    properties: {
      templateId: {
...
export const getHelpTool = {
  name: "get_help",
  description:
    "Get interactive help for a specific topic. Topics include getting_started, database_connection, querying, schema_management, performance, troubleshooting, and advanced features.",
  input_schema: {
    type: "object",
    properties: {
      topic: {
        type: "string",
        description:
          "Help topic: getting_started, database_connection, querying, schema_management, performance, troubleshooting, advanced",
      },
    },
    required: ["topic"],
  },
...
export const searchHelpTool = {
  name: "search_help",
  description:
    "Search help content for keywords. Returns relevant help topics ranked by relevance.",
  input_schema: {
    type: "object",
    properties: {
      query: {
...
export const startTourTool = {
  name: "start_tour",
  description:
    "Start an interactive guided tour of the MCP server features. Perfect for new users to learn the basics step-by-step.",
  input_schema: {
    type: "object",
    properties: {},
  },
...
export const getRateLimitsTool = {
  ...
  description:
    "Get current rate limit status including remaining requests, tier information, and reset times.",
  input_schema: {
    type: "object",
    properties: {
      clientId: {
...
export const setClientTierTool = {
  ...
  description:
    "Set the rate limit tier for a client. Tiers: free (60/min), pro (300/min), enterprise (1000/min).",
  input_schema: {
    type: "object",
    properties: {
      clientId: {
...
export const registerTenantTool = {
  ...
  description:
    "Register a new tenant for multi-tenant isolation. Supports schema, database, or row-level isolation strategies.",
  input_schema: {
    type: "object",
    properties: {
      tenantId: {
...
export const listTenantsTool = {
  name: "list_tenants",
  description:
    "List all registered tenants with their statistics and configuration.",
  input_schema: {
    type: "object",
    properties: {
      tier: {
...
export const listPluginsTool = {
  name: "list_plugins",
  description:
    "List all registered plugins including their status and metadata.",
  input_schema: {
    type: "object",
    properties: {},
  },
...
export const getMetricsTool = {
  name: "get_metrics",
  description:
    "Get all metrics in JSON or Prometheus format. Includes HTTP requests, database queries, cache stats, circuit breaker status, and more.",
  input_schema: {
    type: "object",
    properties: {
      format: {
...
export const analyzeQueryTool = {
  name: "analyze_query",
  description:
    "Analyze query performance using EXPLAIN ANALYZE. Provides execution time, query plan, and optimization suggestions including missing indexes and inefficient operations.",
  input_schema: {
    type: "object",
    properties: {
      sql: {
...
export const getOptimizationReportTool = {
  ...
  description:
    "Get comprehensive query optimization report including slow queries, suggested indexes, and performance statistics.",
  input_schema: {
    type: "object",
    properties: {},
  },
    ...
    switch (toolName) {
      ...
      default:
        throw new MCPError(
          ...
          `Unknown advanced tool: ${toolName}`,
          {},
        );
    }
  } catch (error) {
      ...
      content: [
        {
          type: "text",
          text: JSON.stringify(
            { success: true, message: `Cleared ${cleared} entries` },
            null,
            2,
          ),
        },
...
async function handleExecuteTemplate(args, context) {
  const compiled = context.templateEngine.compile(
    args.templateId,
    args.params || {},
  );
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { success: true, message: `Tier set to ${args.tier}` },
          null,
          2,
        ),
      },
...
async function handleRegisterTenant(args, context) {
  const tenant = await context.multiTenancyManager.registerTenant(
    args.tenantId,
    args,
  );
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { success: true, tenant: tenant.toJSON() },
          null,
          2,
        ),
      },
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { success: true, message: "Plugin enabled" },
          null,
          2,
        ),
      },
    ...
    content: [
      {
        type: "text",
        text: JSON.stringify(
          { success: true, message: "Plugin disabled" },
          null,
          2,
        ),
      },
...
async function handleAnalyzeQuery(args, context) {
  const analysis = await context.queryOptimizer.analyzeQuery(
    args.sql,
    args.params || [],
  );
