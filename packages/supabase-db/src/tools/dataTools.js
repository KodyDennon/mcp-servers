...
export const importDataTool = {
  name: "importData",
  description:
    "Import data into a table from various formats (CSV or JSON). Supports up to 10,000 rows per batch. Uses secure parameterized queries to prevent SQL injection.",
...
export const insertRowTool = {
  name: "insertRow",
  description:
    "Insert a new row into a table. Returns the inserted row including any auto-generated fields.",
  input_schema: z.object({
...
export const updateRowTool = {
  name: "updateRow",
  description:
    "Update an existing row in a table by ID. Returns the updated row.",
  input_schema: z.object({
    tableName: z.string().min(1).describe("The name of the table"),
    rowId: z
...
async function handleUpdateRow(client, input) {
  ...
  if (result.rows.length === 0) {
    throw new MCPError(
      "RESOURCE_NOT_FOUND",
      `Row with id ${rowId} not found in ${safeTableName}`,
      {
        table: safeTableName,
        row_id: rowId,
