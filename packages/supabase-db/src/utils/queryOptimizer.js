  ...
  generateSuggestions(plan, sql) {
    ...
    if (planNode["Actual Rows"] > 10000) {
      suggestions.push({
        ...
        severity: "medium",
        message:
          "Query returns a large number of rows. Consider adding LIMIT or WHERE clauses.",
    ...
    if (planNode["Rows Removed by Filter"]) {
      ...
      if (filterEfficiency < 10) {
        suggestions.push({
          ...
          severity: "medium",
          message:
            "Filter is removing most rows. Consider rewriting the query or adding an index.",
    ...
    if (
      ...
      suggestions.push({
        ...
        severity: "high",
        message:
          "Nested loop with large outer relation. Consider using hash join or merge join.",
        rows: planNode["Actual Rows"],
      });
    }
    ...
    if (
      ...
      suggestions.push({
        ...
        severity: "critical",
        message:
          "JOIN without ON clause detected. This may cause a cartesian product.",
      });
    }
    ...
    if (sql.toUpperCase().includes("SELECT *")) {
      suggestions.push({
        ...
        severity: "low",
        message:
          "Using SELECT * retrieves all columns. Specify only needed columns for better performance.",
      });
    }
  ...
  suggestIndexes() {
    ...
    for (const query of this.slowQueries) {
      ...
      if (joinMatch) {
        ...
        tableColumns.set(
          `${table}.${col1}`,
          (tableColumns.get(`${table}.${col1}`) || 0) + 1,
        );
      }
    }
