...
export function mapPostgresError(error, context = {}) {
    ...
    switch (errorCode) {
        ...
        case "28000": // invalid_authorization_specification
            return new MCPError("AUTH_INVALID_CREDENTIALS", "Database authentication failed. Check your credentials.", {
                postgres_code: errorCode,
                hint: "Verify POSTGRES_URL_NON_POOLING environment variable",
                docs_url: "https://github.com/KodyDennon/mcp-servers/tree/main/packages/supabase-db#setup",
            });
        case "42P01": // undefined_table
            return new MCPError("DB_TABLE_NOT_FOUND", `Table not found: ${context.tableName || "unknown"}`, {
                postgres_code: errorCode,
        ...
        case "23514": // check_violation
            return new MCPError("DB_CONSTRAINT_VIOLATION", `Database constraint violation: ${errorMessage}`, {
                postgres_code: errorCode,
        ...
        case "42804": // datatype_mismatch
            return new MCPError("DB_QUERY_FAILED", `SQL syntax error: ${errorMessage}`, {
                postgres_code: errorCode,
                query: context.query,
                hint: "Check SQL syntax and data types",
            });
        case "53300": // too_many_connections
            return new MCPError("DB_CONNECTION_FAILED", "Too many database connections. Please retry.", {
                postgres_code: errorCode,
                retry_after_ms: 1000,
        ...
        case "08000": // connection_exception
            return new MCPError("DB_CONNECTION_FAILED", "Database connection failed. Please check your connection settings.", {
                postgres_code: errorCode,
                retry_after_ms: 2000,
                hint: "Verify POSTGRES_URL_NON_POOLING is correct and database is accessible",
            });
        case "57014": // query_canceled
            return new MCPError("NETWORK_TIMEOUT", "Query execution timeout. Try reducing the query scope.", {
                postgres_code: errorCode,
                retry: true,
        ...
        default:
            ...
            if (errorMessage.includes("timeout")) {
                return new MCPError("NETWORK_TIMEOUT", "Operation timed out. Please retry.", {
            ...
            if (errorMessage.includes("ECONNREFUSED") ||
                ...
                return new MCPError("DB_CONNECTION_FAILED", "Cannot connect to database. Check your connection URL.", {
                    original_error: errorMessage,
                    hint: "Verify POSTGRES_URL_NON_POOLING environment variable",
                });
            }
            if (errorMessage.includes("permission denied")) {
                return new MCPError("AUTH_INVALID_CREDENTIALS", "Permission denied. Check database user permissions.", {
                    original_error: errorMessage,
                    hint: "Ensure database user has required permissions",
                });
            }
            // Generic error fallback
            return new MCPError("INTERNAL_ERROR", errorMessage || "An unexpected error occurred", {
                original_error: errorMessage,
                stack: error.stack,
                context: context,
            });
...
export function validateEnvironment(required = []) {
    ...
    if (missing.length > 0) {
        throw new MCPError("AUTH_MISSING_ENV_VARS", `Missing required environment variables: ${missing.join(", ")}`, {
...
export function sanitizeIdentifier(identifier) {
    if (!identifier || typeof identifier !== "string") {
        throw new MCPError("VALIDATION_INVALID_INPUT", "Invalid identifier provided", {
            identifier: identifier,
            hint: "Identifier must be a non-empty string",
        });
    }
    // Check for SQL injection patterns
    const dangerous = /[;\-\-\/\*]|(\bDROP\b|\bDELETE\b|\bTRUNCATE\b|\bEXEC\b|\bEXECUTE\b)/i;
    if (dangerous.test(identifier)) {
        throw new MCPError("VALIDATION_SQL_INJECTION", "Potentially dangerous SQL detected in identifier", {
            identifier: identifier,
    ...
    if (!/^[a-zA-Z0-9_\.]+$/.test(cleaned)) {
        throw new MCPError("VALIDATION_INVALID_INPUT", "Invalid identifier format", {
            identifier: identifier,
            hint: "Use only alphanumeric characters, underscores, and dots",
        });
    }
