...
export function parseCSV(csvData, options = {}) {
    const { delimiter = ",", quote = '"', escape = "\\", skipEmptyLines = true, maxRows = 10000, } = options;
    if (!csvData || typeof csvData !== "string") {
        throw new MCPError("VALIDATION_INVALID_INPUT", "CSV data must be a non-empty string", {
    ...
    const invalidColumns = columns.filter((col) => !col || typeof col !== "string" || col.trim() === "");
    if (invalidColumns.length > 0) {
        throw new MCPError("VALIDATION_INVALID_INPUT", "CSV contains empty or invalid column names", {
            columns: columns,
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (rows.length >= maxRows) {
            throw new MCPError("RESOURCE_LIMIT_EXCEEDED", `CSV exceeds maximum row limit of ${maxRows}`, {
                max_rows: maxRows,
...
export function validateCSV(csvData, options = {}) {
    try {
        ...
        return {
            valid: false,
            error: error instanceof MCPError ? error.error : { message: error.message },
        };
    }
}
