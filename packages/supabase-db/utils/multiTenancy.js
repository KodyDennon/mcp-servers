...
export class MultiTenancyManager {
    ...
    async registerTenant(tenantId, options = {}) {
        ...
        console.error(`✅ Tenant registered: ${tenantId} (${this.isolationStrategy} isolation)`);
        return tenant;
    }
    /**
    ...
    async provisionTenant(tenant) {
        ...
        switch (this.isolationStrategy) {
            ...
            case "row":
                ...
                break;
            default:
                throw new MCPError("VALIDATION_INVALID_INPUT", `Unknown isolation strategy: ${this.isolationStrategy}`, {});
        }
    }
    /**
    ...
    async createTenantSchema(pool, tenant) {
        ...
        try {
            ...
            await client.query(`ALTER SCHEMA "${tenant.schema}" OWNER TO CURRENT_USER`);
    ...
    async createTenantDatabase(tenant) {
        if (!tenant.connectionString) {
            throw new MCPError("VALIDATION_INVALID_INPUT", "Connection string required for database isolation", {});
        }
        // Add connection for this tenant
        await this.connectionManager.addConnection(tenant.connectionString, `tenant_${tenant.id}`);
    ...
    addTenantFilter(sql, tenantId) {
        ...
        if (upperSql.includes("WHERE")) {
            ...
            return sql.replace(/FROM/i, `FROM ... WHERE tenant_id = '${tenantId}' AND`);
        }
        return sql;
    }
    /**
