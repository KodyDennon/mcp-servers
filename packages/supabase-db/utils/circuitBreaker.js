    ...
    async execute(fn, fallback = null) {
        ...
        if (this.state === CircuitState.OPEN) {
            if (Date.now() < this.nextAttempt) {
                this.totalRejected++;
                const error = new MCPError("NETWORK_UNAVAILABLE", "Circuit breaker is open - service temporarily unavailable", {
                    state: this.state,
    ...
    async executeWithTimeout(fn) {
        return Promise.race([
            ...
            new Promise((_, reject) => {
                setTimeout(() => {
                    reject(new MCPError("NETWORK_TIMEOUT", "Operation timed out", {
    ...
    transitionTo(newState) {
        ...
        switch (newState) {
            case CircuitState.OPEN:
                ...
                console.error(`🔴 Circuit breaker opened after ${this.failures} failures. Will retry in ${this.resetTimeout}ms`);
                break;
...
export class SmartConnectionPool {
    ...
    async connect() {
        ...
        return this.circuitBreaker.execute(async () => {
            ...
            throw new MCPError("DB_CONNECTION_FAILED", "Unable to connect to database - circuit breaker open", {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                if (attempt < retries) {
                    ...
                    console.error(`Retry attempt ${attempt + 1}/${retries} after ${delay}ms...`);
    ...
    getStats() {
        return {
            ...
            connections: {
                ...
                success_rate: this.connectionAttempts > 0
                    ? Math.round((this.connectionSuccesses / this.connectionAttempts) * 100)
                    : 0,
            },
            queries: {
                total: this.queryCount,
                errors: this.queryErrors,
                error_rate: this.queryCount > 0
