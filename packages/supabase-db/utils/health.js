...
export class HealthMonitor {
    ...
    getHealthStats() {
        ...
        const healthyChecks = this.healthHistory.filter((h) => h.status === HealthStatus.HEALTHY).length;
        const degradedChecks = this.healthHistory.filter((h) => h.status === HealthStatus.DEGRADED).length;
        const unhealthyChecks = this.healthHistory.filter((h) => h.status === HealthStatus.UNHEALTHY).length;
        const avgDuration = this.healthHistory.reduce((sum, h) => sum + (h.duration_ms || 0), 0) /
            totalChecks;
        return {
...
export function createHealthCheckTool(connectionManager) {
    ...
    return {
        name: "health_check",
        description: "Run comprehensive health diagnostics on the MCP server and database connection. Returns detailed status including database connectivity, memory usage, response times, and connection pool health.",
        input_schema: {
            type: "object",
            properties: {
                include_history: {
