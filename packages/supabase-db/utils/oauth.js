...
export class OAuthConfig {
    ...
    validate() {
        ...
        if (!this.authUrl)
            missing.push("authUrl");
        if (!this.tokenUrl)
            missing.push("tokenUrl");
        if (missing.length > 0) {
            throw new MCPError("AUTH_MISSING_ENV_VARS", "OAuth configuration is incomplete", {
...
export class OAuthFlow {
    ...
    async authorize() {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                this.stopServer();
                reject(new MCPError("NETWORK_TIMEOUT", "OAuth authorization timeout", {
                    timeout_seconds: 300,
            ...
            `);
                        this.stopServer();
                        reject(new MCPError("AUTH_INVALID_CREDENTIALS", `OAuth authorization failed: ${error}`, {
                            error: error,
            ...
            `);
                        this.stopServer();
                        reject(new MCPError("AUTH_INVALID_CREDENTIALS", "OAuth state mismatch", {
            ...
            this.server.on("error", (error) => {
                clearTimeout(timeout);
                reject(new MCPError("NETWORK_UNAVAILABLE", `Failed to start OAuth callback server: ${error.message}`, {
                    port: port,
    ...
    async exchangeCodeForToken(code) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new MCPError("AUTH_INVALID_CREDENTIALS", `Token exchange failed: ${response.status} ${response.statusText}`, {
            ...
            throw new MCPError("NETWORK_UNAVAILABLE", `Failed to exchange code for token: ${error.message}`, {
    ...
    async refreshAccessToken() {
        if (!this.refreshToken) {
            throw new MCPError("AUTH_INVALID_CREDENTIALS", "No refresh token available", {
        ...
        try {
            ...
            if (!response.ok) {
                throw new MCPError("AUTH_TOKEN_EXPIRED", "Failed to refresh access token", {
                    status: response.status,
            ...
            throw new MCPError("NETWORK_UNAVAILABLE", `Failed to refresh token: ${error.message}`, {
...
export class SimpleAuth {
    ...
    getHeaders() {
        switch (this.type) {
            case "api_key":
                if (!this.key) {
                    throw new MCPError("AUTH_MISSING_ENV_VARS", "API key not configured", {
            ...
            case "bearer":
                if (!this.key) {
                    throw new MCPError("AUTH_MISSING_ENV_VARS", "Bearer token not configured", {
                        hint: "Set API_KEY environment variable",
                    });
                }
                return {
            ...
            case "basic":
                if (!this.username || !this.password) {
                    throw new MCPError("AUTH_MISSING_ENV_VARS", "Basic auth credentials not configured", {
                        missing_vars: [
            ...
            default:
                throw new MCPError("VALIDATION_INVALID_INPUT", `Unknown auth type: ${this.type}`, {
