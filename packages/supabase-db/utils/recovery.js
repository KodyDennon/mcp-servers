...
export class AutoRecoveryManager {
    ...
    async attemptRecovery(error, context = {}) {
        ...
        if (attempts >= this.maxRecoveryAttempts) {
            console.error(`⛔ Max recovery attempts (${this.maxRecoveryAttempts}) reached for ${errorPattern}`);
            return false;
        }
        this.recoveryAttempts.set(key, attempts + 1);
        console.error(`🔄 Attempting auto-recovery for ${errorPattern} using ${strategy} (attempt ${attempts + 1}/${this.maxRecoveryAttempts})`);
        try {
    ...
    async resetCircuitBreaker(context) {
        ...
        try {
            if (this.connectionManager.smartPool &&
...
export class GracefulShutdown {
    ...
    async shutdown(exitCode = 0) {
        ...
        const timeout = setTimeout(() => {
            console.error(`⏰ Shutdown timeout (${this.shutdownTimeout}ms) exceeded, forcing exit`);
            process.exit(1);
...
export class ConnectionWatchdog {
    ...
    async checkConnection() {
        try {
            ...
            console.error(`⚠️  Watchdog detected connection failure (${this.consecutiveFailures}/${this.maxConsecutiveFailures}): ${error.message}`);
            if (this.consecutiveFailures >= this.maxConsecutiveFailures) {
                console.error("🚨 Max consecutive failures reached, triggering recovery");
