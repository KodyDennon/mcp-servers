...
export class TokenBucketLimiter {
    ...
    getStats() {
        const clients = Array.from(this.buckets.entries()).map(([id, bucket]) => ({
            ...
            utilizationPercent: Math.round(((this.capacity - bucket.tokens) / this.capacity) * 100),
        }));
        return {
            capacity: this.capacity,
...
export class SlidingWindowLimiter {
    ...
    getStats() {
        const clients = Array.from(this.windows.entries()).map(([id, window]) => ({
            ...
            utilizationPercent: Math.round((window.requests.length / this.maxRequests) * 100),
        }));
        return {
            maxRequests: this.maxRequests,
...
export class RateLimitManager {
    constructor(options = {}) {
        ...
        for (const [tierName, config] of Object.entries(this.tiers)) {
            this.limiters.set(`${tierName}-minute`, new SlidingWindowLimiter({
                maxRequests: config.requestsPerMinute,
                windowMs: 60000,
            }));
            this.limiters.set(`${tierName}-hour`, new SlidingWindowLimiter({
                maxRequests: config.requestsPerHour,
                windowMs: 3600000,
            }));
            this.limiters.set(`${tierName}-burst`, new TokenBucketLimiter({
    ...
    getStats() {
        ...
        stats.clientTiers = Array.from(this.clientTiers.entries()).map(([id, tier]) => ({
            clientId: id,
            tier: tier,
        }));
        return stats;
