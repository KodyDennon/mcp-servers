...
const ENV_DESCRIPTIONS = {
    POSTGRES_URL_NON_POOLING: "PostgreSQL connection string (non-pooling, direct)",
    ...
    OPENAI_API_KEY: "OpenAI API key for AI/RAG tools (OPTIONAL - only needed if using rag, indexDirectory, or indexUrl tools)",
};
/**
...
export function loadEnvConfig() {
    ...
    console.error(`[DEBUG] process.env.POSTGRES_URL after dotenv: ${process.env.POSTGRES_URL}`);
    console.error(`[DEBUG] process.env.POSTGRES_URL_NON_POOLING after dotenv: ${process.env.POSTGRES_URL_NON_POOLING}`);
    console.error(`[DEBUG] process.env.SUPABASE_URL after dotenv: ${process.env.SUPABASE_URL}`);
}
/**
...
export async function ensureEnvironment({ allowInteractive = true } = {}) {
    ...
    const friendlyList = missing
        ...
        .join(", ");
    throw new Error(`Missing required environment variables: ${friendlyList}. ` +
...
export async function promptForMode() {
    ...
    if (choice.trim() === "2") {
        ...
        const execChoice = await question("Select execution mode (1 or 2) [default: 1]: ");
    ...
    if (await envFileExists()) {
        ...
        envContent = envContent
            .split("\n")
            .filter((line) => !line.startsWith("MCP_MODE=") &&
