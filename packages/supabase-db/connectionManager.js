...
import { AutoRecoveryManager, GracefulShutdown, ConnectionWatchdog, } from "./utils/recovery.js";
    ...
    async addConnection(connectionString, id = null) {
        ...
        try {
            ...
            if (this.enableReliability && wrappedPool.executeWithRetry) {
                ...
                result = await client.query("SELECT current_database(), current_user, version()");
            }
            if (client) {
                client.release();
            }
            ...
            console.error(`❌ Failed to establish connection ${connectionId}:`, error.message);
            // Attempt auto-recovery if enabled
            if (this.enableAutoRecovery && this.recoveryManager) {
