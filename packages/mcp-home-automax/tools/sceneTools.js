...
export function getSceneTools() {
    return [
        ...
        },
        {
            ...
            description: "Get environmental context and summary including current time, active devices, and aggregate sensor data",
            inputSchema: {
                type: "object",
                properties: {
                    includeDeviceStates: {
 ...
 */
export async function handleSceneToolCall(name, args, homeGraph, adapterManager, policyEngine) {
    switch (name) {
        case "home_list_scenes": {
            const { tag } = args;
            let scenes = homeGraph.getAllScenes();
            if (tag) {
                scenes = scenes.filter((s) => s.tags?.some((t) => t.toLowerCase() === tag.toLowerCase()));
            }
            return {
                content: [
                    {
        ...
        case "home_get_context": {
            const { includeDeviceStates } = args;
            const now = new Date();
            ...
            const activeDevices = allDevices.filter((device) => {
                const switchCap = device.capabilities.find((c) => c.type === "switch" || c.type === "light");
                return switchCap && switchCap.state && switchCap.state.on;
            });
            ...
            const tempSensors = sensors.filter((s) => s.type === "temperature" &&
                typeof s.state.temperature === "number");
            let avgTemperature;
            if (tempSensors.length > 0) {
                const sum = tempSensors.reduce((acc, s) => acc + (s.state.temperature || 0), 0);
            ...
            if (includeDeviceStates) {
                context.deviceStates = allDevices.map((d) => ({
                    id: d.id,
                    name: d.name,
                    type: d.type,
                    online: d.online,
        ...
        case "home_set_group": {
            const { groupId, action, parameters } = args;
            const snapshot = homeGraph.getSnapshot();
            ...
            for (const device of devices) {
                try {
                    ...
                    errors.push(`${device.name}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            return {
                content: [
